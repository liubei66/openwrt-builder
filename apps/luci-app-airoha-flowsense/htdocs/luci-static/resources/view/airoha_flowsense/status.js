...
/* â”€â”€ Drop-delta tracking (all counters are cumulative since interface up) â”€â”€ */
var _prevPseDrops    = null;
var _prevCdmHwfDrops = null;
var _prevBridgeDrops = null;
var _prevPpeBnd      = null;  // for tachometer heartbeat
var _maxUnbSeen      = 8;     // UNB scale denominator â€?only grows, never shrinks
...
function formatPleCount(free) {
	if (typeof free !== 'number' || free < 0) return 'â€?;
...
/* â”€â”€ CPU Frequency State (used by CPU/NPU tachometer) â”€â”€ */
function freqBarState(hw, min, max, pll, gov) {
	var pll_khz = (pll || 0) * 1000;
	// cpufreq sysfs missing (e.g. AN7581 broken DVFS) â€?fall back to PLL hardware read
	if (!hw && pll_khz > 0)
...
function renderPpePanels(ppe) {
	...
	return E('div', { 'style': 'display:flex;gap:16px;align-items:flex-start' }, [
		renderPpePanel('BND â€?Hardware Offloaded', '#00c8ff', 'BND', 'label-success', bnd.entries, bnd.total, true),
		renderPpePanel('UNB â€?Pending / Learning', '#4caf50', 'UNB', '', unb.entries, unb.total, false)
	]);
}
...
/* â”€â”€ PPE Tachometer (embedded inside compass inner fill) â”€â”€ */
function buildTachoInner(ppe, cs, mode) {
	...
	var TICKS = 90; // 4Â° per tick
	var cx = 150, cy = 150;
	// BND inner (anti-CW): 1 flow = 1 tick, max 90
	var bndLit = Math.min(TICKS, bndTot);
	// UNB outer (CW): sticky power-of-2 scale â€?grows when new peak is seen, never shrinks
	...
	// Ring labels â€?at 9 and 3 o'clock inside the BND inner ring (r<56 from centre)
	p.push('<text x="107" y="153" text-anchor="middle" fill="#00c8ff" font-size="7" font-family="monospace" opacity="0.75">â—„BND</text>');
	p.push('<text x="193" y="153" text-anchor="middle" fill="#ff9800" font-size="7" font-family="monospace" opacity="0.75">UNBâ–?/text>');
	...
	//   BND inner anti-clockwise: r=57â€?7
	//   UNB outer clockwise:      r=71â€?1
	...
	// Centre readout â€?mode + status at top, BND count large, IPv4/IPv6 split, UNB below
...
function buildPpeTerminalBody(ppe) {
	...
	// BND section
	s += sp(cyn,'â–?BND') + '  ' + sp(wht, bndTot+' flows');
	...
	// UNB section
	s += sp(org,'â–?UNB') + '  ' + sp(wht, unbTot+' flows') + '\n';
	s += sp(sep, divLine) + '\n';
	...
	s += '\n';
	s += sp(grn,'root@OpenWrt') + sp(mute,':~# ') + '<span class="ppe-cursor" style="'+wht+'">â–?/span>';
	return s;
}

function renderPpeTerminal(ppe) {
	var bar = E('div', { 'class': 'ppe-terminal-bar' }, [
		E('span', { 'class': 'ppe-terminal-title' }, 'ppe_monitor  â€? root@OpenWrt:~')
	]);
...
function needleTip(latencyMs) {
	// Full west arc 120Â°â†?40Â° (120Â° sweep). Log scale so low-latency range is sensitive.
	// 0ms = 120Â° (lower-left, good), 100ms = 240Â° (upper-left, bad/near north)
	var clamped = Math.min(Math.max(latencyMs||0, 0), 100);
	var logPct = Math.log(clamped + 1) / Math.log(101); // 0â†?, 100msâ†?
...
/* â”€â”€ HW Buffer Health (replaces SQM â€?NPU traffic bypasses qdisc entirely) â”€â”€ */
function hwBufferState(fe, ppe, mode) {
	fe = fe || {}; ppe = ppe || {}; mode = mode || 'router';

	// PSE port drops: cumulative across all internal ports (0-9).
	// These include CDM/PPE internal paths that drop normally â€?not a reliable
	...
	// CDM HW-forwarding drops â€?frames the NPU forwarded that CDM couldn't accept.
	...
	// Delta since last poll â€?null on first call (baseline only, no alarm)
	...
	// PPE offload efficiency â€?BND/(BND+UNB). Shown in subtitle for info only.
...
function compassState(bypass, hwBuf, jitter, wan, wifi, bridge, mode) {
	...
	// Latency â€?jitter daemon pings upstream and works in both router and AP mode
	var latMs = jitter.last_ping || 0;

	// Integrity / errors
	var errCount = 0;
	var eastAlarm = false;
	var worstSignal = 0;  // dBm â€?0 means no data; always negative when valid
	...
	if (mode === 'router') {
		...
		// min_signal = worst (lowest dBm) station on that band â€?most meaningful
...
function buildCompassSVG(cs, mode, ppe) {
	...
	// North CW 215â†?25: text curves along top, reads Lâ†’R
	var tpN = 'M 35.3 69.7 A 140 140 0 0 1 264.7 69.7';
	// South CCW 150â†?0: text curves along bottom, reads Lâ†’R
	var tpS = 'M 30.5 219.0 A 138 138 0 0 0 269.5 219.0';
	// East CW 300â†?0: text curves along right side, reads topâ†’bottom
	var tpE = 'M 219.0 30.5 A 138 138 0 0 1 219.0 269.5';
	// West CCW 240â†?20: text curves along left side, reads topâ†’bottom
	...
	// Quadrant labels â€?curved textPath following each arc
	...
	// PPE state ring outside compass disc â€?cyan=BND present, invisible otherwise
	'<circle id="cp-ppe-glow" cx="150" cy="150" r="150" fill="none" stroke="'+ppeRing.color+'" stroke-width="5" style="'+ppeRing.style+'"/>' +
	// Solid silver outer ring â€?outermost dashboard border
...
/* â”€â”€ CPU/NPU Load Tachometer â”€â”€ */
function buildCpuNpuTacho(cs, ppe, st, ti) {
	...
	// CPU frequency â€?same source as the existing freq bar
	...
	p.push('<text x="107" y="153" text-anchor="middle" fill="#ffe066" font-size="7" font-family="monospace" opacity="0.75">â—„LOAD</text>');
	p.push('<text x="193" y="153" text-anchor="middle" fill="#00cc44" font-size="7" font-family="monospace" opacity="0.75">FREQâ–?/text>');
	...
	// PLE pool â€?curved textPath at r=91, south arc (CCW 160Â°â†?0Â°), outside the freq ring.
	...
	p.push('<text font-size="8" font-family="monospace" fill="'+pleH.color+'" opacity="0.9" letter-spacing="1"><textPath href="#cn-ple-arc" startOffset="50%" text-anchor="middle">PLE '+pleStr+' â—?'+pleH.text+'</textPath></text>');
...
function _cnPpeRingStyle(ppe, ti) {
	...
	// drops toward zero â€?that's the precursor signal for mt7996 SER / TX wedge.
...
function buildCpuNpuCompassSVG(cs, ppe, st, ti) {
	...
	// Outer border â€?tight around the tachometer content
	...
	// Solid silver outer ring â€?sits outside the PPE/PLE glow ring
...
function buildWifiBandTacho(bandIdx, ws, qType, bndCount, unbCount) {
	...
	// Effective throughput: link capacity Ã— success rate (1 âˆ?retry fraction)
	...
	var retryLit = Math.round(retryVal / 100 * TICKS); // 0â€?00% maps to 0â€?0 ticks
	...
	p.push('<text x="107" y="153" text-anchor="middle" fill="'+retryCol+'" font-size="7" font-family="monospace" opacity="0.75">â—„RTY</text>');
	p.push('<text x="193" y="153" text-anchor="middle" fill="'+accent+'" font-size="7" font-family="monospace" opacity="0.75">TXâ–?/text>');
	...
	// UNB count arc at 12 o'clock (curved textPath, r=91, CW 200Â°â†?40Â°)
	...
	// BND count arc at 6 o'clock (curved textPath, r=91, CCW 160Â°â†?0Â°) â€?opposite health arc
	...
	// Centre readout â€?retry % above band name
	if (retryVal > 0)
...
function buildWifiTachoElements(wifi, ti, st, ppe) {
	...
	var elems = [];
	// 6 GHz â†?5 GHz â†?2.4 GHz (band index 2 â†?1 â†?0)
	for (var b = 2; b >= 0; b--) {
		var ws = null;
...
/* â”€â”€ Ethernet Port Horizontal Bar Gauges â”€â”€ */
...
function renderCompassCards(cs, bypass, jitter, wan, wifi, bridge, mode) {
	...
	var northSub   = mode === 'ap'
		? 'CPU: '+cs.cpuPct+'%  |  Bridge drops Î”: '+bridgeDelta
	...
	var southSub   = 'PSE Î”: '+hb.pseDelta+' CDM Î”: '+hb.cdmHwfDelta+' | PPE: '+hb.ppePct+'% BND ('+hb.ppeBound+'/'+hb.ppeTotal+')';

	// West card: Latency
	var latVal   = cs.latMs > 0 ? cs.latMs.toFixed(1)+'ms' : (jitter.available===false ? 'N/A' : '---');
	var latColor = cs.latColor;
	var latSub   = 'Jitter: '+(jitter.jitter||0).toFixed(1)+'ms  |  '+(jitter.samples||0)+' samples  |  Ping: '+(jitter.target||'223.5.5.5')+' âœ?;
...
function updateCompassCards(cs, bypass, jitter, wan, wifi, bridge, mode) {
	...
	setCard(divs[0], cs.npuActive?'ACTIVE':(cs.hwEnabled?'IDLE':'CPU PATH'),
		...
		mode==='ap'
			?'CPU: '+cs.cpuPct+'%  |  Bridge drops Î”: '+bridgeDelta2
			:'CPU: '+cs.cpuPct+'%  |  WAN: '+cs.wanMbps+' Mbps');
	...
	setCard(divs[2], latVal, cs.latColor,
		'Jitter: '+(jitter.jitter||0).toFixed(1)+'ms  |  '+(jitter.samples||0)+' samples  |  Ping: '+latTarget+' âœ?);
	...
	setCard(divs[3],
		...
		hb.color||'#00cc44',
		'PSE Î”: '+hb.pseDelta+' CDM Î”: '+hb.cdmHwfDelta+' | PPE: '+hb.ppePct+'% BND ('+hb.ppeBound+'/'+hb.ppeTotal+')');
}
...
return view.extend({
	...
	render: function(data) {
		...
		// Compass SVG container â€?tachometer is embedded inside (innerHTML so we can update by element ID)
		...
		// Data fetch + DOM update function â€?called immediately and via poll
		var fetchData = L.bind(function() {
			return Promise.all([
				...
				// Ethernet port gauges â€?compute per-port Mbps deltas from cumulative byte counters
